...
esp_err_t i2c_master_write_slave(i2c_bus_handle_t bus, uint8_t addr, uint8_t reg, uint8_t *data, uint32_t len)
{
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    ...
    if(data != NULL)
        i2c_master_write(cmd, data, len, I2C_ACK_CHECK_EN);
    i2c_master_stop(cmd);
    i2c_bus_t* i2c_bus = (i2c_bus_t*)bus;
    esp_err_t err = i2c_bus_cmd_begin(bus, cmd);
    i2c_cmd_link_delete(cmd);
    return err;
}